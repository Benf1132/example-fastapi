...
def test_get_all_posts(authorized_client, test_posts):
    ...
    # assert posts_list[0].Post.id == test_posts[1].id
...
@pytest.mark.parametrize("title, content, published", [
    ...
    ("title2", "content 2", True),
    ("title3", "content 3", True),
])
...
def test_create_post_default_published(authorized_client, test_user, test_posts):
    res = authorized_client.post(f"/posts/", json={"title": "test", "content": "test"})

    created_post = schemas.Post(**res.json())
    assert res.status_code == 201
    assert created_post.title == "test"
    assert created_post.content == "test"
    assert created_post.published == True
    assert created_post.owner_id == test_user['id']

def test_unauthorized_user_create_post(client, test_user, test_posts):
    res = client.post(f"/posts/", json={"title": "test", "content": "test"})
    assert res.status_code == 401
